...
only the filter values parameterized â€” there's no user input in the SQL
...
# "highest paid" worked â€” the builder was narrow and nobody could see it from the gate.
TOP_PAID_PHRASES = (
...
# Everything above, plus the aggregate and status words â€” what a question may contain
...
# A question can name a constraint these fixed templates have no way to express â€”
...
def unsupported_constraints(text, known_names=()):
    ...
    `known_names` are values the database actually holds â€” product names, customer
...
# so the digits in "only 1" / "top 1" don't trip the "a specific number" refusal â€”
...
def reference_names(ref):
    """Every value get_reference_data() found, flattened â€” see unsupported_constraints."""
...
def detect_aggregate(text):
    ...
    # not to count rows â€” but only when there's an amount in it to total.
...
def find_match(options, text):
    for option in options:
        ...
        if len(option) <= 3:
            ...
            # was dropped in silence â€” every employee came back as the answer.
...
def build_departments_query(_text, aggregate, _ref):
    ...
    # â€” how many departments â€” was answered with the full list. The explanation then
