...
The DSN is persisted as plain text in instance/uploads/postgres_connection.json so
the app can reconnect across requests. That's a known, documented trade-off â€”
see the "Known limitations" section in README.md â€” not something to "fix" by
...
from query_assistant.config import POSTGRES_CONFIG_PATH as CONFIG_PATH
from query_assistant.config import UPLOAD_DIR
...
def _friendly_connection_error(exc):
    ...
    if "password authentication failed" in message or "authentication failed" in message:
        return "Authentication failed â€” check the username and password in your connection string."
    if "could not translate host name" in message or "name or service not known" in message:
        return "Couldn't resolve the host â€” check the host part of your connection string."
    if "timeout expired" in message or "timed out" in message:
        return "Connection timed out â€” check the host/port, and that your network can reach it."
    if "connection refused" in message:
        return "Connection refused â€” check the host and port are correct and the database is reachable."
    if 'database "' in message and "does not exist" in message:
        return "That database name doesn't exist on this server â€” check the connection string."
...
def _resolves_to_private_address(host):
    ...
    resolve is treated as non-private â€” the connection attempt will fail on its
...
def check_dsn_target(dsn):
    ...
    if host is None:
        ...
        raise ValueError(
            ...
            "disabled by default â€” set ALLOW_PRIVATE_DB_HOSTS=true to allow it."
        )
...
def _table_names_with(cur):
    """Just the names â€” no row counts, no columns."""
    cur.execute(
...
def get_table(table_name):
    ...
    a COUNT(*) over every other table in the database first â€” on a real PostgreSQL
