"""Lets a user attach an external SQLite file (instance/uploads/connected.db) and
query it the same way as the built-in database. Only one external SQLite or
PostgreSQL connection is active at a time â€” see src/query_assistant/app.py:get_active_source.
"""

import os
import sqlite3

from query_assistant.config import CONNECTED_DB_PATH, UPLOAD_DIR
...
def list_tables():
    """Every table with its row count and columns â€” for the connect page, which shows them."""
    conn = get_connection()
    if conn is None:
        return []
    try:
...
def get_table(table_name):
    ...
    export, and it used to go through list_tables() â€” counting the rows of every
