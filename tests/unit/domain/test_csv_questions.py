...
has no such engine â€” csv_engine only knows the column names and their types â€” so the
...
class TestAggregates:
    ...
    def test_how_much_is_a_sum_not_a_count(self, conn, dataset):
        """"revenue kitna hai" asks how much, and "kitna" is a counting word â€” but
...
class TestWithoutColumnTypes:
    """app.py has the types, but the signature keeps them optional â€” without them the
...
class TestTheStreamTheRouteActuallyGets:
    ...
    it in io.TextIOWrapper raised AttributeError â€” every upload failed on 3.10 and
