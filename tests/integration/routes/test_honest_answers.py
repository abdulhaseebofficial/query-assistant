...
These tests pin both halves of the behaviour â€” what it must refuse, and what it must
...
class TestValuesAreNotConstraints:
    ...
    def test_a_real_constraint_beside_such_a_name_is_still_refused(self, conn):
        """Only the name is exempt â€” the rest of the question is still inspected."""
...
class TestFailureIsExplained:
    ...
    def test_an_off_topic_question_is_marked_off_topic(self):
        from query_assistant.services.query_service import describe_failure
...
class TestAiPathEndToEnd:
    """With a provider configured, its SQL should reach the page â€” proving the wiring,
