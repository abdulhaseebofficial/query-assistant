...
as the demo tables â€” so like `users`, it must stay off every whitelist the SQL
...
from query_assistant.repositories import feedback_repository as feedback
...
class TestItStaysOutOfReach:
    ...
    def test_the_ai_engine_cannot_be_told_to_read_it(self):
        from query_assistant.domain.validation.sql_guardrails import BUILTIN_TABLES, validate_select

        assert "feedback" not in BUILTIN_TABLES
        assert validate_select("SELECT * FROM feedback", BUILTIN_TABLES) is None
