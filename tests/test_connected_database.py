...
class TestHostileColumnNames:
    ...
    CSV's headers get sanitised on the way in. A connected database's don't — and
    ...
    def test_the_column_name_is_read_back_intact(self, quoted_column):
        from backend.connectors import sqlite_connector
    ...
    def test_the_generated_sql_escapes_the_quote(self, quoted_column):
        from backend.connectors import sqlite_connector
        from backend.engines.csv_engine import build_custom_query
...
class TestGeneratedSqlSurvivesAnyIdentifier:
    ...
    builder interpolates them, so every shape has to come out as valid SQL — in
