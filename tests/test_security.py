...
from backend.engines.csv_engine import load_csv
from backend.utils.csv_export import _neutralise_formula, build_csv
...
class TestAttachmentFilename:
    ...
    in them — neither belongs in a response header."""
    ...
    def test_dangerous_characters_never_reach_the_header(self, raw):
        from backend.app import _attachment_header

        header = _attachment_header(raw)
    ...
    def test_an_ordinary_name_survives(self):
        ...
        assert _attachment_header("sales_results.csv") == 'attachment; filename="sales_results.csv"'
