...
file is about what must *not* run — and about the fact that it's the same check the
...
class TestOneSetOfRules:
    ...
    def test_the_console_uses_the_engines_validator(self):
        from backend import sql_console
    ...
    def test_both_callers_agree(self, sql):
        from backend.engines.ai_engine import _validate_select

        statement, reason = check_select(sql, BUILTIN_TABLES)

        assert statement is None
        assert reason
        assert _validate_select(sql, BUILTIN_TABLES) is None
    ...
    def test_an_accepted_query_is_identical_either_way(self):
        ...
        assert statement == _validate_select(sql, BUILTIN_TABLES)
