...
catches the failures dialect bugs actually produce — an identity column written in
SQLite's syntax, a `strftime` that doesn't exist, a `?` where `%s` belongs — all of
which would otherwise only surface as a 500 on the deployment.

Reloading backend.db under a patched environment is unavoidable: it reads
...
def postgres_modules(monkeypatch):
    """backend.db and backend.database as they'd be with DATABASE_URL set."""
    import backend.database
    import backend.db

    monkeypatch.setenv("DATABASE_URL", PG_URL)
    db = importlib.reload(backend.db)
    database = importlib.reload(backend.database)
    try:
        yield db, database
    finally:
        ...
        importlib.reload(importlib.import_module("backend.engines.rule_engine"))
...
class TestDateFilters:
    ...
    def test_the_same_four_periods_exist_in_both(self, postgres_modules):
        ...
        importlib.reload(backend.db)
        assert pg_keys == set(backend.db.DATE_FILTERS)
...
class TestGeneratedQueriesAreValidPostgres:
    ...
    def pg_rule_engine(self, postgres_modules, test_database):
        ...
        import backend.engines.rule_engine as rule_engine
